...
/* =================Task Related===================== */
	...
	public function add_new_p_task($data){
		// data insert   
		$p_task_title  = $this->test_form_input_data($data['p_task_title']);
		$p_task_description = $this->test_form_input_data($data['p_task_description']);
		$p_t_start_time = $this->test_form_input_data($data['p_t_start_time']);
		$p_t_end_time = $this->test_form_input_data($data['p_t_end_time']);

		try{
			$add_p_task = $this->db->prepare("INSERT INTO p_task_info (p_t_title, p_t_description, p_t_start_time, 	p_t_end_time, t_user_id) VALUES (:x, :y, :z, :a, :b) ");

			$add_p_task->bindparam(':x', $p_task_title);
			$add_p_task->bindparam(':y', $p_task_description);
			$add_p_task->bindparam(':z', $p_t_start_time);
			$add_p_task->bindparam(':a', $p_t_end_time);

			$add_p_task->execute();

			$_SESSION['P_Task_msg'] = 'Task Add Successfully';

			header('Location: p_task-info.php');
		}catch (PDOException $e) {
			echo $e->getMessage();
		}
	}
		...
		public function update_p_task_info($data, $p_task_id, $user_role){
			$p_task_title  = $this->test_form_input_data($data['p_task_title']);
			$p_task_description = $this->test_form_input_data($data['p_task_description']);
			$p_t_start_time = $this->test_form_input_data($data['p_t_start_time']);
			$p_t_end_time = $this->test_form_input_data($data['p_t_end_time']);
			$p_status = $this->test_form_input_data($data['p_status']);
			...
			try{
				$update_p_task = $this->db->prepare("UPDATE p_task_info SET p_t_title = :x, p_t_description = :y, p_t_start_time = :z, p_t_end_time = :a, p_status = :c WHERE p_task_id = :id ");

				$update_p_task->bindparam(':x', $p_task_title);
				$update_p_task->bindparam(':y', $p_task_description);
				$update_p_task->bindparam(':z', $p_t_start_time);
				$update_p_task->bindparam(':a', $p_t_end_time);
				$update_p_task->bindparam(':c', $p_status);
				$update_p_task->bindparam(':id', $p_task_id);

				$update_p_task->execute();

				$_SESSION['P_Task_msg'] = 'Task Update Successfully';
	...
	public function delete_p_data_by_this_method($sql,$action_id,$sent_po){
		try{
			$delete_p_data = $this->db->prepare($sql);

			$delete_p_data->bindparam(':id', $action_id);

			$delete_p_data->execute();
